  ...
  if (state === 'expanded') {
    return (
      <aside className="fixed inset-0 z-50 bg-paper animate-fade-in flex flex-col overflow-hidden">
        {/* Adjusted padding to prevent top clipping and provide bottom breathing room */}
        <div className="flex-1 overflow-hidden px-6 md:px-12 lg:px-20 pt-10 md:pt-16 pb-0">
        ...
        <div className="flex justify-between items-center px-6 h-14 border-b border-black/5 flex-shrink-0 bg-paper/50 backdrop-blur-sm sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2 text-gray-400">
            ...
            <button onClick={() => onStateChange('collapsed')} className="p-1.5 hover:text-ink transition-colors rounded hover:bg-black/5" title="Collapse">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d={side === 'left' ? "M15.75 19.5L8.25 12l7.5-7.5" : "M8.25 4.5l7.5 7.5-7.5 7.5"} /></svg>
            </button>
          </div>
        </div>
        {/* Container for content and fixed footers */}
