...
const SettingsModal: React.FC<SettingsModalProps> = ({ 
      ...
      >
        ...
        {(tags.length > 0 || tierTags.length > 0) && (
          <div className="mt-2 flex flex-wrap gap-2">
            {tierTags.map((tag) => (
              <span key={`${model.id}-${tag}`} className="px-2 py-0.5 rounded-full bg-ink text-white text-[9px]">
                {tag}
              </span>
            ))}
            {tags.map((tag) => (
              <span
                key={`${model.id}-${tag}`}
                className="px-2 py-0.5 rounded-full border border-black/5 text-[9px] text-gray-500"
              >
                {tag}
              </span>
            ))}
  ...
  useEffect(() => {
    if (isOpen && !availableModels.length) {
      handleFetchModels();
    }
  }, [isOpen]);
