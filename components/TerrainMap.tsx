    ...
    return (
      <div className="h-full flex flex-col p-12 bg-white/60 backdrop-blur-2xl rounded-[3rem] animate-fade-in border border-black/5 shadow-float">
        <div className="flex justify-between items-center mb-16">
          ...
          <div className="text-[10px] text-gray-300 font-bold tracking-widest uppercase">TERRAIN SYNERGY {currentIndex + 1} / {sessionQueue.length}</div>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <div className="text-[10px] px-5 py-2 bg-secondary/10 text-secondary rounded-full mb-8 font-bold uppercase tracking-widest">Cognitive Mastery: {Math.round(current.masteryScore * 100)}%</div>
          ...
          <div className="w-full max-w-sm space-y-4">
             <button onClick={() => { onUpdateLexicon(current.lemma, { familiarity: 3, masteryScore: 1 }); if (currentIndex < sessionQueue.length - 1) setCurrentIndex(c => c+1); else setMode('browse'); }} className="w-full py-6 bg-ink text-white rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-black transition-all shadow-2xl active:scale-95">Mark Mastered</button>
      ...
      <div className="flex-[3] flex flex-col min-w-0">
        ...
        <div className="flex-1 bg-white/40 rounded-[3.5rem] border border-black/5 relative overflow-hidden group">
           <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full p-20 overflow-visible">
              ...
              {view === 'content' && <line x1={simulatedProgress * 100} y1="0" x2={simulatedProgress * 100} y2="100" stroke="rgba(194,142,91,0.3)" strokeWidth="0.4" strokeDasharray="1,1" />}
           ...
           <div className="flex-1 flex flex-col bg-white border border-black/5 rounded-[3rem] overflow-hidden animate-fade-in-up shadow-float">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-2 no-scrollbar">
                 {activeZoneWords.sort((a:any, b:any) => b.count - a.count).map((word: any) => (
                   <div key={word.lemma} className="flex items-center justify-between p-4 hover:bg-accent/5 rounded-[1.5rem] group transition-all cursor-pointer" onClick={() => handleFetchDefinition(word.lemma)}>
                      <div className="flex-1">
                         ...
                         {word.definition && (
                           <p className="text-[11px] font-serif text-gray-400 italic mt-1 line-clamp-1">{word.definition}</p>
                         )}
                      </div>
