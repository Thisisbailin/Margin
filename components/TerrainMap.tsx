    ...
    return (
      <div className="h-full flex flex-col p-12 bg-white/60 backdrop-blur-2xl rounded-[3rem] animate-fade-in border border-black/5 shadow-float">
        <div className="flex justify-between items-center mb-16">
          <button onClick={() => setMode('browse')} className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-400 hover:text-accent">
            ← EXIT STUDY
          </button>
          <div className="text-[10px] text-gray-300 font-bold tracking-widest uppercase">
            TERRAIN SYNERGY {currentIndex + 1} / {sessionQueue.length}
          </div>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <div className="text-[10px] px-5 py-2 bg-secondary/10 text-secondary rounded-full mb-8 font-bold uppercase tracking-widest">
            Cognitive Mastery: {Math.round(current.masteryScore * 100)}%
          </div>
          <h2 className="text-[10rem] font-display text-ink mb-6 tracking-tighter leading-none">{current.lemma}</h2>

          <div className="max-w-xl mb-12 min-h-[3rem]">
            {current.definition ? (
          ...
          <div className="w-full max-w-sm space-y-4">
            <button
              onClick={() => {
                onUpdateLexicon(current.lemma, { masteryScore: 1 });
                onReviewLemma(current.lemma);
                if (currentIndex < sessionQueue.length - 1) setCurrentIndex((c) => c + 1);
                else setMode('browse');
              }}
              className="w-full py-6 bg-ink text-white rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-black transition-all shadow-2xl active:scale-95"
            >
              Mark Mastered
            </button>
            <button onClick={() => setMode('browse')} className="w-full py-4 text-gray-400 font-bold uppercase tracking-widest text-[10px] hover:text-ink transition-colors">
              Return
            </button>
          </div>
        </div>
      ...
      <div className="flex-[3] flex flex-col min-w-0">
        <div className="flex justify-between items-end mb-12">
          <div>
              ...
              {(['content', 'memory', 'reality'] as TopographyView[]).map((v) => (
                <button
                  ...
                  className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-all relative ${
                    view === v ? 'text-accent' : 'text-gray-300 hover:text-gray-400'
                  }`}
                >
                  {v} topo
            ...
            <h3 className="text-5xl font-display text-ink tracking-tight">Linguistic Terrain</h3>
          </div>
          ...
          <div className="w-72 text-right">
            ...
            <div className="relative h-1 bg-gray-100 rounded-full cursor-pointer group">
              ...
              <div
                className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-2 border-accent rounded-full shadow-md"
                style={{ left: `${simulatedProgress * 100}%` }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-white/40 rounded-[3.5rem] border border-black/5 relative overflow-hidden group">
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full p-20 overflow-visible">
            ...
            {view === 'content' && <path d={`${curvePath} L 100,100 L 0,100 Z`} fill="url(#curveGradient)" className="transition-all duration-1000" />}
            {view === 'content' && (
      ...
      <div className="flex-1 flex flex-col gap-6 w-[420px] pb-4">
            ...
            {[
              ...
              <button
                ...
                className={`p-6 rounded-[2rem] border text-left transition-all group relative overflow-hidden ${
                  selectedZone === r.id ? 'border-accent bg-white shadow-float scale-[1.02]' : 'border-black/5 bg-surface/40 hover:bg-white'
                }`}
              >
        ...
        {selectedZone ? (
          <div className="flex-1 flex flex-col bg-white border border-black/5 rounded-[3rem] overflow-hidden animate-fade-in-up shadow-float">
            <div className="p-8 border-b border-black/5 flex justify-between items-center bg-paper/10">
              <div>
                ...
                <p className="text-[9px] text-gray-400 uppercase tracking-widest mt-1">Linguistic Analysis</p>
              </div>
              <button
                onClick={() => startStudy(activeZoneWords)}
                className="px-6 py-2.5 bg-ink text-white rounded-full font-bold uppercase tracking-widest text-[10px] hover:bg-accent transition-all shadow-lg active:scale-95"
              >
                Study Segment
              </button>
            </div>
                ...
                .map((word: any) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {word.definition && <p className="text-[11px] font-serif text-gray-400 italic mt-1 line-clamp-1">{word.definition}</p>}
                    </div>
          ...
          <div className="flex-1 bg-surface/20 p-12 rounded-[3rem] border border-black/5 border-dashed flex flex-col justify-center items-center text-center opacity-30">
            <p className="text-xs font-serif text-gray-500 italic max-w-[200px]">Choose a zone above to begin journey through terrain.</p>
          </div>
        )}
      </div>
    </div>
