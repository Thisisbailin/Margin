...
const HomeMeditation: React.FC<HomeMeditationProps> = ({
  userId,
  projectId,
  bookId,
  isActive = true,
  ...
  useEffect(() => {
    if (!isActive) return;
    loadList();
  }, [isActive, userId, projectId, bookId, status, search]);
  ...
  }, [isActive, activeId, userId, projectId, bookId, status, search]);
  ...
  return (
    <div className={`flex-1 flex flex-col overflow-hidden animate-fade-in ${className || ''}`.trim()}>
      <div className={`flex-1 h-full w-full rounded-[3rem] border border-black/5 bg-white/60 shadow-soft overflow-hidden ${panelClassName || ''}`.trim()}>
        <div className="h-full flex flex-col gap-6 p-6 md:p-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="font-display text-4xl text-ink italic">Meditation</h1>
