...
const MarginSidebar: React.FC<MarginSidebarProps> = ({ 
  ...
  const findLastIndex = (
    items: AgentMessage[],
    predicate: (item: AgentMessage) => boolean,
  ...
  const lastAgentIndex = findLastIndex(messages, (msg) => msg.role === 'agent');
  const lastUserIndex = findLastIndex(messages, (msg) => msg.role === 'user');
  const lastUserBeforeAgentIndex =
    lastAgentIndex !== -1 ? findLastIndex(messages, (msg) => msg.role === 'user', lastAgentIndex) : -1;

  const visibleIndices = new Set<number>();
  if (lastAgentIndex !== -1) {
    visibleIndices.add(lastAgentIndex);
    if (lastUserBeforeAgentIndex !== -1) {
      visibleIndices.add(lastUserBeforeAgentIndex);
    }
  } else if (lastUserIndex !== -1) {
    visibleIndices.add(lastUserIndex);
  }

  const visibleMessages = messages.filter((_, index) => visibleIndices.has(index));
  const priorMessages = messages.filter((_, index) => !visibleIndices.has(index));
      ...
      <div className="flex-1 space-y-12 pb-12">
        ...
        {priorMessages.length > 0 && (
          <details className="pl-5">
            <summary className="text-[9px] uppercase tracking-[0.3em] text-faded cursor-pointer select-none">
              历史记录 · {priorMessages.length}
            </summary>
            <div className="mt-6 space-y-12">
              {priorMessages.map(renderMessage)}
            </div>
          </details>
        )}
